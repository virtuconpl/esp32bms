...
#define SAMPLE_DEVICE_NAME          "ESP_SPP_SERVER_R1"    //The Device Name Characteristics in GAP
...
#define BMS_FRAME_MAX_LEN   256

static QueueHandle_t bms_uart_queue = NULL;
static SemaphoreHandle_t bms_response_ready;  // Will be initialized in bms_uart_init()
static uint8_t bms_response[256];
static int bms_response_len = 0;

// BMS request frame (status request)
static const uint8_t bms_request[] = {0x68, 0x31, 0xCE, 0x68, 0x02, 0x02, 0x5C, 0x60, 0x8F, 0x16};
static const int bms_request_len = sizeof(bms_request);
...
static const uint8_t spp_adv_data[26] = {
    ...
    0x11, ESP_BLE_AD_TYPE_NAME_CMPL, 'E', 'S', 'P', '_', 'S', 'P', 'P', '_', 'S', 'E', 'R','V', 'E', 'R', '_', 'R', '1'
};
...
// BMS UART Functions
static void bms_uart_init(void)
{
    uart_config_t uart_config = {
    ...
    // Install UART driver
    uart_driver_install(UART_PORT_NUM, UART_RX_BUF_SIZE, 256, 10, &bms_uart_queue, 0);
    uart_param_config(UART_PORT_NUM, &uart_config);
    uart_set_pin(UART_PORT_NUM, UART_TX_PIN, UART_RX_PIN, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
    
    // Create binary semaphore for response signaling
    bms_response_ready = xSemaphoreCreateBinary();
    
    ESP_LOGI(GATTS_TABLE_TAG, "BMS UART initialized on port %d, baud %d", UART_PORT_NUM, UART_BAUD_RATE);
}

void bms_uart_task(void *pvParameters)
{
    uint8_t rx_buffer[BMS_FRAME_MAX_LEN];
    uart_event_t event;
    
    ESP_LOGI(GATTS_TABLE_TAG, "BMS UART task started - waiting for responses");
    
    for (;;) {
        if (xQueueReceive(bms_uart_queue, (void*)&event, (TickType_t)portMAX_DELAY)) {
            if (event.type == UART_DATA) {
                int rx_len = uart_read_bytes(UART_PORT_NUM, rx_buffer, event.size, portMAX_DELAY);
                
                ESP_LOGI(GATTS_TABLE_TAG, "BMS RX %d bytes", rx_len);
                ESP_LOG_BUFFER_HEX("BMS_RX", rx_buffer, rx_len);
                
                // Store response in global buffer
                if (rx_len > 0 && rx_len < BMS_FRAME_MAX_LEN) {
                    memcpy(bms_response, rx_buffer, rx_len);
                    bms_response_len = rx_len;
                    
                    // Signal that response is ready
                    if (bms_response_ready != NULL) {
                        xSemaphoreGive(bms_response_ready);
                    }
                }
            }
        }
...
void spp_test_notification_task(void * arg)
{
    uint8_t notif_msg[256];
    int counter = 0;
    ...
    for(;;) {
        ...
        ESP_LOGI(GATTS_TABLE_TAG, "=== BMS POLL #%d ===", counter);
        ESP_LOGI(GATTS_TABLE_TAG, "is_connected=%d, enable_data_ntf=%d", is_connected, enable_data_ntf);
        
        if (is_connected) {
            // Send request to BMS
            ESP_LOGI(GATTS_TABLE_TAG, "BMS Request TX %d bytes", bms_request_len);
            ESP_LOG_BUFFER_HEX("BMS_REQ", (uint8_t*)bms_request, bms_request_len);
            uart_write_bytes(UART_PORT_NUM, (const char*)bms_request, bms_request_len);
            
            // Wait for response with 2 second timeout
            BaseType_t result = xSemaphoreTake(bms_response_ready, pdMS_TO_TICKS(2000));
            
            ESP_LOGI(GATTS_TABLE_TAG, "Semaphore result: %d", result);
            
            if (result == pdTRUE && bms_response_len > 0) {
                // Send response via BLE
                ESP_LOGI(GATTS_TABLE_TAG, "BMS Response sent via BLE, %d bytes", bms_response_len);
                esp_ble_gatts_send_indicate(spp_gatts_if, spp_conn_id,
                                           spp_handle_table[SPP_IDX_SPP_DATA_NTY_VAL],
                                           bms_response_len, bms_response, false);
                bms_response_len = 0;  // Reset for next cycle
            } else {
                // Send "*" if no response
                notif_msg[0] = '*';
                ESP_LOGI(GATTS_TABLE_TAG, "BMS No response - sending '*' via BLE");
                esp_ble_gatts_send_indicate(spp_gatts_if, spp_conn_id,
                                           spp_handle_table[SPP_IDX_SPP_DATA_NTY_VAL],
                                           1, notif_msg, false);
            }
        } else {
            ESP_LOGI(GATTS_TABLE_TAG, "Not connected - skipping BMS poll");
        }
    }
    vTaskDelete(NULL);
}
...
static void spp_task_init(void)
{
    ...
    // Initialize BMS UART
    bms_uart_init();
    xTaskCreate(bms_uart_task, "bms_uart_task", 4096, NULL, 10, NULL);
    
    // BMS test/notification task - wysyła request co 5 sekund
    xTaskCreate(spp_test_notification_task, "bms_test_task", 2048, NULL, 10, NULL);
}
